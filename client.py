...
layout = [
    [sg.Text("Name:"), sg.InputText(key="-NAME-"), sg.Button("Join")],
    [sg.Multiline(key="-CHAT-", size=(30, 20), disabled=True)],
    [sg.InputText(key="-MSG-"), sg.Button("Send"), sg.Button("Leave")]
]
...
def receive_message_from_server(sck, m):
    ...
    while True:
        ...
        if "has joined the chat." in from_server or " has left the chat." in from_server:
